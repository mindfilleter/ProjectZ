...
class WanderingNPC(NPC):
    def __init__(
        self, game, spawn, boundaries: pygame.Rect, *groups
    ) -> None:
        super().__init__(spawn.x, spawn.y, *groups)

        try:
            with resources.path("projectz.assets", "player.png") as sheet_path:
                spritesheet = pygame.image.load(sheet_path).convert_alpha()
        except FileNotFoundError:
            print("Error: player.png not found. Using green square placeholder.")
            spritesheet = pygame.Surface(
                (game.TILE_SIZE, game.TILE_SIZE), pygame.SRCALPHA
            )
            spritesheet.fill((0, 255, 0))

        frame_rect = pygame.Rect(32, 0, game.TILE_SIZE, game.TILE_SIZE)
        ...
        dialog_keys = sorted([key for key in spawn.properties if key.startswith("dialog_")])
    ...
    def update(self, collision_rects):
        ...
        if not self.path and (self.target is None or pygame.time.get_ticks() - self.path_timer > self.path_timeout):
            self.target = self.find_new_target()
            if self.target:
                self.path = self.pathfinder.get_path(self.rect.center, self.target)
        ...
        if self.path:
            ...
            if direction.length() < 2: # A small threshold
                self.path.pop(0)
                if not self.path: # If path is now empty, we've reached the destination
                    self.target = None
                    return
                else:
                    next_point = pygame.math.Vector2(self.path[0])
    ...
    def find_new_target(self):
        ...
        while attempts < 20: # Try up to 20 times to find a valid spot
            rand_x = random.randint(self.boundaries.left, self.boundaries.right - tile_size)
            rand_y = random.randint(self.boundaries.top, self.boundaries.bottom - tile_size)
            ...
            target_rect = pygame.Rect(target_x - tile_size // 2, target_y - tile_size // 2, tile_size, tile_size)

            # Ensure the target is within the wandering boundaries and not in a wall
            if self.boundaries.contains(target_rect) and not self.collidable.game.map.is_wall(target_x, target_y):
